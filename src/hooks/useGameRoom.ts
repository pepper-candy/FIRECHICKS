  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // Store interval for cleanup in the outer scope
        (window as any).__supabaseHostPingInterval = pingInterval;
      } catch (error) {
    ...
    return () => {
      alive = false;
      const pingInterval = (window as any).__supabaseHostPingInterval;
      if (pingInterval) {
...
export function useWebRTCRoomBroadcast(roomCode: string) {
  ...
  useEffect(() => {
    ...
    // Re-track periodically to keep WebRTC broadcast alive - every 1 second
    const heartbeat = window.setInterval(async () => {
      if (alive && subscribedRef.current) {
        try { 
          await channel.track({ roomCode, ts: Date.now(), source: 'webrtc' }); 
        } catch {}
      }
...
/**
 * Fetches rooms from Neon Postgres database via API endpoint
 * Polls the API every 3 seconds to get active rooms
 */
    ...
    const fetchRooms = async () => {
      try {
        ...
        if (Array.isArray(data)) {
          ...
          console.warn('API /api/rooms returned non-array response:', data);
        }
      } catch (error) {
        if (isMounted) {
