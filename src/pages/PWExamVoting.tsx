...
type Phase = "menu" | "questions" | "camera" | "voting";
type VotingPhase = "submission" | "voting" | "results";

interface PWExamVotingProps {
  isSubmitter?: boolean;
...
const PWExamVoting = ({
  ...
  const [votingPhase, setVotingPhase] = useState<VotingPhase>("submission");
  ...
  const [currentVote, setCurrentVote] = useState<'pass' | 'fail' | null>(null);
  ...
  const handleVote = (vote: 'pass' | 'fail') => {
    if (!hasVoted) {
      ...
      toast.success(`You voted: ${vote.toUpperCase()}`);
    }
  };
  ...
  if (phase === "voting") {
    if (!votingState) {
      return (
        <div className="flex flex-col items-center justify-center min-h-screen p-6 gap-8">
          ...
          <h1 className="text-lg md:text-2xl text-primary text-glow-green tracking-wider">FINAL EXAM</h1>
          <p className="text-xs text-muted-foreground font-mono">Waiting for voting data...</p>
        </div>
      );
    }
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen p-6 gap-4">
        ...
        {/* Show submission box if this player is the submitter and we're in submission phase */}
        {isSubmitter && votingPhase === "submission" && (
        ...
        {/* Show voting UI in voting phase */}
        {votingPhase === "voting" && (
          <ExamVotingUI
            submitterName={submitterName}
            maskedAnswer={votingState.maskedAnswer}
            startedAt={votingState.startedAt}
